...
class Main:
	...
	def run(self):
		...
		while True:
			...
			already_bet = bet_complete

			# Check if 120 minutes has passed yet.
			if time_since_collect > 7200:
				irc.send_message(self, '#saltyteemo', '!collect')
				timers['!collect'] = time()
			...
			if time_since_first_bet >= 170 and betting_started and not bet_complete:
				...
				irc.send_message(self, '#saltyteemo', '!%s %s' % (underdog, bet))
			...
			irc.check_for_ping(self, data)

			# Check if most recent data is a message from Twitch chat.
			if irc.check_for_message(data):
				message_dict = irc.get_message(self, data)
				...
				if username != config['username']:
					...
					if username == 'xxsaltbotxx':
						...
						if 'Bet complete' in message:
							...
							if already_bet:
								print('Your bot bet on %s' % underdog)
				...
				# 						irc.send_message(self, '#saltyteemo', result)
				...
				# 				general.pbot('(%s) (%s)' % (command, username), '#saltyteemo')
				# 				res = commands.get_return(command)
				# 				general.pbot(res, channel)
				# 				irc.send_message(self, '#saltyteemo', res)
				# 				commands.update_last_used(command, '#saltyteemo')
